...
def per_query_error2(W, A, eps=np.sqrt(2), delta=0, normalize=False):
    """
    modification version of per_query_error, removed the var/delta part and get direct diag()
    so the math behid per_query_error is that W @ AtA1 @ W.T == W@A1@A1.T@W.T 
    At current situation pcost is 1 
    """
    W, A = convert_implicit(W), convert_implicit(A)
    if isinstance(W, VStack):
        return np.concatenate([per_query_error2(Q, A, eps, delta, normalize) for Q in W.matrices])
    delta = A.sensitivity()
    var = 2.0/eps**2
    AtA1 = A.gram().pinv()
    X = W @ AtA1 @ W.T
...
def per_query_error_sampling2(W, A, number=100000, eps=np.sqrt(2), normalize=False):
    """
    ...
    W, A = convert_implicit(W), convert_implicit(A)
    if isinstance(W, Weighted):
        ans = W.weight**2 * per_query_error_sampling2(W.base, A, number)
    #elif isinstance(W, VStack) and type(A) == VStack:
    #    m = W.shape[0]
    #    num = lambda Wi: int(number*Wi.shape[0]/m + 1)
    #    samples = [per_query_error_sampling(Wi,Ai.base,num(Wi)) for Wi,Ai in zip(W.matrices,A.matrices)]
    ...
    #    ans = np.concatenate([err/w**2 for w, err in zip(weights, samples)])
        ...
        m = W.shape[0]
        num = lambda Wi: int(number*Wi.shape[0]/m + 1)
        samples = [per_query_error_sampling2(Wi, A, num(Wi)) for Wi in W.matrices]
        ans = np.concatenate(samples)
    elif isinstance(W, Kronecker) and isinstance(A, Kronecker):
        assert isinstance(A, Kronecker)
        pieces=[per_query_error_sampling2(Wi, Ai, number) for Wi,Ai in zip(W.matrices,A.matrices)]
        ans = np.prod(pieces, axis=0)
    elif isinstance(W, Kronecker) and isinstance(A, workload.Marginals):
        # optimization: if W is Marginals, all errors are the same
        if all( type(Wi) in [workload.Identity, workload.Ones] for Wi in W.matrices ):
        ...
        ans = np.random.choice(per_query_error2(W, A), number)
        delta = A.sensitivity()
    #ans *= 2.0/eps**2
    return np.sqrt(ans) if normalize else ans
